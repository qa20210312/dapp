...
function release_top5($starttime,$endtime,$method){
    ...
    $toper=array();
    while(true){
        $url=$eventServerHost."/v1/contracts/".$tron->fromHex($contract)."/events?event_name=Deposit&limit=200&fingerprint=$fingerprint&min_block_timestamp=$starttime&max_block_timestamp=$endtime";
        $content=json_decode(file_get_contents($url),true);
        foreach ($content['data'] as $record){
            $toper[$record['result']['recommender_addr']]+=(int)$record['result']['_value'];
        }
...
function save_top5(){
    ...
    while(true){
        $url=$eventServerHost."/v1/contracts/".$tron->fromHex($contract)."/events?event_name=Deposit&limit=200&fingerprint=$fingerprint&min_block_timestamp=$starttime";
